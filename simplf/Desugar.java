...
public Stmt visitForStmt(Stmt.For stmt) {
    Stmt initStmt = (stmt.init != null) ? new Stmt.Expression(stmt.init) : null;

    List<Stmt> whileBody = new ArrayList<>();
    whileBody.add(stmt.body);
    if (stmt.incr != null) {
        whileBody.add(new Stmt.Expression(stmt.incr));
    }

    Stmt whileLoop = new Stmt.While(stmt.cond, new Stmt.Block(whileBody));
